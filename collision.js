...
function circlevcircle(center1, radius1, center2, radius2){
    return center1.subtract(center2).modulus() <= radius1 + radius2;
}

function circlevaabb(center, radius, topleft, width, height){

    return center1.subtract(center2).modulus() <= radius1 + radius2;
}

function aabbvaabb(topleft1, size1, topleft2, size2){
...
function rectvrect(rect1, rect2){
...
function linevline(x1, y1, x2, y2, x3, y3, x4, y4){
...
function lsvsaabb(start, end, rect){
...
function inaabb(pos, x, y, w, h){
    return (pos.y >= y && pos.y <= y + h) && (pos.x >= x && pos.x <= x + w);
}

function InBox( hit, topleft, bottomright, axis) {
...
function linevaabb(topleft, size, start, end, hit){
    //debugger;
...
function linevcircle(center, radius, start, end){
...
function linevellipse(center, size, start, end, d=5){
